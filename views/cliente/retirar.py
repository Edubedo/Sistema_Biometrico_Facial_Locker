...
def _get_locker_estado(id_locker) -> str | None:
    """Consulta el estado actual del locker en la BD."""
    try:
...
class ScanLine(QWidget):
    ...
    def paintEvent(self, event):
        ...
        p.drawRoundedRect(0, 0, w, h, h//2, h//2); p.end()
...
QLabel#ok    { color: #B9EA89; font-size: 13px; font-weight: 700; font-family: 'Segoe UI', sans-serif; }
QLabel#err   { color: #ff6b6b; font-size: 13px; font-weight: 700; font-family: 'Segoe UI', sans-serif; }
...
QLabel#inline_result_badge {
    background: rgba(210,229,252,1.0); color: #1f5fb8;
...
QLabel#inline_result_title  { color: #2f80ed; font-size: 26px; font-weight: 900; font-family: 'Segoe UI'; }
QLabel#inline_result_subtitle { color: #000000; font-size: 11px; font-family: 'Segoe UI'; }
QLabel#inline_result_detail {
    color: #000000; font-size: 30px; font-weight: 900;
...
QPushButton#btn_action_red {
    ...
    padding: 14px 16px; font-size: 16px; font-weight: 800; font-family: 'Segoe UI', sans-serif;
}
...
QLabel#step_main_text {
    color: #dceaff; font-size: 18px; font-weight: 700; font-family: 'Segoe UI', sans-serif;
}
...
_STEP_KEYS        = ["ret.step1", "ret.step2", "ret.step3", "ret.step4"]
_STEP_DURATION_MS = 900  # era 1400 → ~2 s menos de espera


class StepOverlay(QWidget):
    """
    Overlay que muestra los pasos del proceso antes de iniciar el escaneo.
    Avanza automáticamente. Al terminar el último emite `finished`.
    """
    finished = pyqtSignal()
...
class ActionTile(QWidget):
    ...
    def __init__(self, mode: str, title: str, image_name: str, parent=None):
        super().__init__(parent)
        self.mode = mode; self.title = title; self.image_name = image_name
        self._hovered = False; self._pressed = False
        self.setCursor(Qt.PointingHandCursor)
        self.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        if mode == "take":
            self._accent=QColor(217,11,11); self._accent_soft=QColor(255,107,107,45)
            self._card_bg=QColor(16,30,62); self._icon_bg=QColor(120,20,20); self._icon_bg2=QColor(217,11,11)
        else:
            self._accent=QColor(25,168,91); self._accent_soft=QColor(185,234,137,45)
            self._card_bg=QColor(16,30,62); self._icon_bg=QColor(42,124,50); self._icon_bg2=QColor(25,168,91)
    ...
    def paintEvent(self, _):
        p = QPainter(self); p.setRenderHint(QPainter.Antialiasing)
        W, H = self.width(), self.height(); pad = _dp(10)
        for off in range(4, 0, -1):
            s = QColor(0,0,0,int(80*(off/4))); p.setPen(Qt.NoPen); p.setBrush(QBrush(s))
        ...
        path = QPainterPath(); path.addRoundedRect(card_rect, _dp(18), _dp(18))
        p.setClipPath(path)
        ...
        if self._hovered:
            rg = QRadialGradient(W//2, int(H*0.38), int(min(W,H)*0.55))
            rg.setColorAt(0.0, self._accent_soft); rg.setColorAt(1.0, QColor(0,0,0,0))
            p.setBrush(QBrush(rg)); p.setPen(Qt.NoPen); p.drawRect(pad, pad, W-pad*2, H-pad*2)
        p.setClipping(False)
        bp = QPen(self._accent.lighter(120) if self._hovered else QColor(73,118,190,160),
        ...
        icon_size=int(min(W,H)*0.34); cx=W//2; cy=int(H*0.36)
        ...
        p.setPen(Qt.NoPen); p.setBrush(QBrush(ig)); p.drawRoundedRect(icon_rect, _dp(16), _dp(16))
        shim=QLinearGradient(cx-icon_size, cy-icon_size, cx+icon_size, cy-icon_size)
        shim.setColorAt(0.0,QColor(255,255,255,0)); shim.setColorAt(0.5,QColor(255,255,255,28)); shim.setColorAt(1.0,QColor(255,255,255,0))
        p.setBrush(QBrush(shim)); p.drawRoundedRect(icon_rect, _dp(16), _dp(16))
        p.setPen(QPen(self._accent.lighter(140), _dp(1.2))); p.setBrush(Qt.NoBrush)
        p.drawRoundedRect(icon_rect.adjusted(0.5,0.5,-0.5,-0.5), _dp(16), _dp(16))
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
        pix = QPixmap(os.path.join(project_root, "public", self.image_name))
        if not pix.isNull():
        ...
        font=QFont("Segoe UI", max(10, int(H*0.060)), QFont.Bold); p.setFont(font)
        p.setPen(QPen(QColor(220,235,255)))
        p.drawText(QRectF(pad, label_top, W-pad*2, int(H*0.24)),
                   Qt.AlignHCenter|Qt.AlignTop|Qt.TextWordWrap, self.title)
        p.end()
...
class InlineResultCard(QWidget):
    def __init__(self, parent=None):
        ...
        self.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        root = QVBoxLayout(self); root.setContentsMargins(0,0,0,0); root.setAlignment(Qt.AlignCenter)
        self._card = QFrame(); self._card.setObjectName("inline_result_card")
        self._card.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        self._card.setMaximumWidth(480); self._card.setMinimumHeight(420)
        cl = QVBoxLayout(self._card); cl.setContentsMargins(48,40,48,40); cl.setSpacing(18); cl.setAlignment(Qt.AlignCenter)
        self._icon=QLabel("✓"); self._icon.setAlignment(Qt.AlignCenter); self._icon.setFixedSize(100,100)
        self._icon.setStyleSheet("background:rgba(47,128,237,0.12);color:#2f80ed;border-radius:50px;font-size:40px;font-weight:900;font-family:'Segoe UI';")
        cl.addWidget(self._icon, alignment=Qt.AlignCenter)
        self._badge=QLabel(""); self._badge.setObjectName("inline_result_badge"); self._badge.setAlignment(Qt.AlignCenter); self._badge.setFixedHeight(26)
        cl.addWidget(self._badge, alignment=Qt.AlignCenter)
        self._title=QLabel(""); self._title.setObjectName("inline_result_title"); self._title.setAlignment(Qt.AlignCenter); self._title.setWordWrap(True)
        cl.addWidget(self._title)
        self._subtitle=QLabel(""); self._subtitle.setObjectName("inline_result_subtitle"); self._subtitle.setAlignment(Qt.AlignCenter); self._subtitle.setWordWrap(True)
        cl.addWidget(self._subtitle)
        self._divider=QFrame(); self._divider.setStyleSheet("background:rgba(135,178,235,0.90);border:none;min-height:1px;max-height:1px;")
        cl.addWidget(self._divider)
        self._detail=QLabel(""); self._detail.setObjectName("inline_result_detail"); self._detail.setAlignment(Qt.AlignCenter); self._detail.setWordWrap(True)
        cl.addWidget(self._detail)
        self._footer=QLabel(""); self._footer.setAlignment(Qt.AlignCenter); self._footer.setWordWrap(True)
        self._footer.setStyleSheet("color:#24405f;font-size:12px;font-family:'Segoe UI';")
    ...
    def show_result(self, kind, title, subtitle, detail=""):
        accent={"ok_blue":"#2f80ed","ok":"#2e7d32","warn":"#e66c00","err":"#c62828"}.get(kind,"#2f80ed")
        badge={"ok_blue":tr("result.ok"),"ok":tr("result.ok"),"warn":tr("result.warn"),"err":tr("result.err")}.get(kind,tr("result.ok"))
        self._icon.setStyleSheet(f"background:rgba(47,128,237,0.12);color:{accent};border-radius:50px;font-size:40px;font-weight:900;font-family:'Segoe UI';")
        self._badge.setText(badge)
        self._badge.setStyleSheet(f"background:rgba(210,229,252,1.0);color:{accent};border:1px solid rgba(135,178,235,0.85);border-radius:10px;font-size:8px;font-weight:800;font-family:'Segoe UI';letter-spacing:3px;padding:4px 16px;")
        self._title.setText(title)
        self._title.setStyleSheet(f"color:{accent};font-size:26px;font-weight:900;font-family:'Segoe UI';letter-spacing:-0.5px;")
        self._subtitle.setText(subtitle)
        self._detail.setText(detail); self._detail.setVisible(bool(detail))
        self._divider.setVisible(bool(detail)); self.setVisible(True)

    def clear(self):
        self._title.clear(); self._subtitle.clear(); self._detail.clear(); self._footer.clear(); self.setVisible(False)
...
class RetirarPage(QWidget):
    ...
    def __init__(self):
        ...
        # Auto-reintento de escaneo después de "rostro no reconocido"
        # Solo reintenta en ese caso (no cuando no hay sesión activa)
        ...
        self.detect_card = QFrame(); self.detect_card.setObjectName("detect_card")
        self.detect_card.setVisible(False)
        detect_l = QVBoxLayout(self.detect_card)
        detect_l.setContentsMargins(16,14,16,14); detect_l.setSpacing(6); detect_l.setAlignment(Qt.AlignCenter)
        detect_title_row = QHBoxLayout(); detect_title_row.setSpacing(8); detect_title_row.setAlignment(Qt.AlignCenter)
        self.detect_icon = QLabel("✓"); self.detect_icon.setAlignment(Qt.AlignCenter)
        self.detect_icon.setFixedSize(28,28)
        self.detect_icon.setStyleSheet("background:rgba(185,234,137,0.18);color:#B9EA89;border-radius:14px;font-size:16px;font-weight:900;font-family:'Segoe UI';")
        self.detect_title_lbl = lbl(tr("ret.detect_title"), "h3", Qt.AlignCenter)
        detect_title_row.addWidget(self.detect_icon); detect_title_row.addWidget(self.detect_title_lbl)
        ...
        self.opts = QFrame(); self.opts.setObjectName("actions_card")
        ...
        btn_row.addWidget(self.btn_retirar, 1); btn_row.addWidget(self.btn_seguir, 1)
        ol.addLayout(btn_row, 1)
        self.opts.setVisible(False)
        rl.addWidget(self.opts, 1)

        self.result_card = InlineResultCard(); rl.addWidget(self.result_card, 1); self.result_card.setVisible(False)
        body.addWidget(right, 1); root.addLayout(body, 1)
    ...
    def _show_steps(self):
        """Posiciona el overlay de pasos y lo arranca."""
    ...
    def _auto_retry(self):
        """
        Reintento automático después de 'rostro no reconocido'.
        Solo se dispara para ese error — no para 'sin sesión activa'.
        Limpia el estado y vuelve a mostrar los pasos de instrucciones.
        """
    ...
    def _on_recognized(self, face_uid):
        ...
        if face_uid == CamThread.CAMERA_ERROR:
            ...
            # Error de cámara: no reintentar automáticamente (problema de hardware)
            return
        ...
        if not face_uid:
            ...
            # Reintento automático: tal vez fue un frame malo o movimiento
    ...
    def _do_retirar(self):
        if not self._id_sesion or self._action_done:
            return
        ...
        # en gpio_locker.py. No debe activarse en beep_start_scan() ni beep_success().
        current_estado = _get_locker_estado(self._id_locker)
        if current_estado != "abierto":
            ...
            print(f"[INFO] _do_retirar: locker {num_locker} ya abierto, se omite abrir_locker()")
    ...
    def _do_seguir(self):
        ...
        if current_estado != "abierto":
            ...
            print(f"[INFO] _do_seguir: locker {num_locker} ya abierto, se omite abrir_locker()")
